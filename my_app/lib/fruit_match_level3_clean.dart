...
class _FruitMatchLevel3GameState extends State<FruitMatchLevel3Game> with TickerProviderStateMixin {
  ...
  void _handleSliceSelection(SliceFruit slice) {
    if (droppedSlices.length >= 3) return; // Limit to 3 slices
    
    setState(() {
      draggedSlice = slice.name;
    });
  }
  
  void _handleSliceDrop(String sliceName) {
    if (draggedSlice == null) return;
    
    setState(() {
      droppedSlices.add(draggedSlice!);
      availableSlices.removeWhere((slice) => slice.name == draggedSlice);
      score += 10;
      
      // Update bowl image based on first dropped slice
      if (droppedSlices.length == 1) {
        if (draggedSlice!.contains('Mango')) {
          currentBowlImage = 'assets/Mango_Slice.png';
        } else if (draggedSlice!.contains('Papaya')) {
          currentBowlImage = 'assets/Papaw_slice.png';
        } else if (draggedSlice!.contains('Banana')) {
                          ...
                          child: ConstrainedBox(
                            ...
                            child: Row(
                              children: [
                                ...
                                // Right side - Drag area (Slice fruits)
                                Expanded(
                                  flex: 1,
                                  child: Container(
                                    margin: const EdgeInsets.all(20),
