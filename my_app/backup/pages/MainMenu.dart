...
class _DownSyndromeDetectionPageState extends State<DownSyndromeDetectionPage> {
  ...
  Future<void> _submitGameScore() async {
    try {
      final response = await http.post(
        ...
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'memory_score': _flipCardScore,
          'reaction_time': _reactionTime,
        }),
      );

      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Game score submitted'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showGameDialog() {
    int localScore = _flipCardScore;
    int localReaction = _reactionTime;
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              title: Text('Cognitive Game'),
              content: SingleChildScrollView(
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Text('Flip Cards Game'),
                    SizedBox(height: 20),
                    Row(
                    ...
                    Text('Memory Score: $localScore', style: TextStyle(fontSize: 18)),
                    SizedBox(height: 20),
                    Text('Reaction Time Test'),
                    Slider(
                      value: localReaction.toDouble(),
                      min: 50,
                      max: 500,
                      divisions: 45,
                      label: localReaction.toString(),
                      onChanged: (value) {
                        setState(() {
                          localReaction = value.round();
                        });
                      },
                    ),
                    Text('${localReaction} ms', style: TextStyle(fontSize: 16)),
                  ],
                ),
              ),
              actions: [
                ...
                TextButton(
                  onPressed: () {
                    this.setState(() {
                      _flipCardScore = localScore;
                      _reactionTime = localReaction;
                      _userProfile['Memory_Score'] = localScore;
                      _userProfile['Reaction_Time'] = localReaction;
                    });
                    _submitGameScore();
                    Navigator.pop(context);
                  },
